...
// mysql-cdc.spec.ts: the trigger model (oracledb.json) has a REQUIRED
// "databaseName" (unlike MySQL's optional "databases" TEXT_SET, which is a
// different field entirely — this one is a plain TEXT/EXPRESSION field) and
// an optional "schemas" TEXT_SET, deliberately left empty for the same
// reason as MySQL's databases field.
export default function createTests() {
    test.describe.serial('Oracle DB CDC Integration Tests', {
    }, async () => {
        initTest();
        const listenerName = 'orderOracledbCdcListener';
        const table = 'orders';

        test('Create Oracle DB CDC Integration', async ({ }, testInfo) => {
            const testAttempt = testInfo.retry + 1;
            console.log('Creating a new service in test attempt: ', testAttempt);

            const artifactWebView = await createArtifactAndGetWebview('Oracle CDC Integration', 'trigger-oracledb');
            ...
            await form.fill({
                values: {
                    databaseName: { type: 'cmEditor', value: '"orderdb"', additionalProps: { switchMode: 'expression-mode' } },
            ...
            await projectExplorer.findItem([DEFAULT_PROJECT_NAME, `Oracle CDC Integration`], 30000);
        ...
        test('Delete Oracle DB CDC Integration', async ({ }, testInfo) => {
            const testAttempt = testInfo.retry + 1;
            console.log('Deleting Oracle DB CDC integration in test attempt: ', testAttempt);

            await getWebview(BI_INTEGRATOR_LABEL, page);
            await deleteArtifactFromTree([DEFAULT_PROJECT_NAME, `Oracle CDC Integration`]);
        });
    });
}
