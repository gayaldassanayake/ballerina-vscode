...
// described in the Solace/RabbitMQ/MQTT/ASB commits blocked verification.
// Structurally near-identical to solace.spec.ts (this connector's own trigger
// model mirrors plain Solace's: listenerVarName/url/messageVpn/auth/
// advancedConfig, with auth a nested CHOICE with no pre-selected default and
// destination a top-level CHOICE defaulting to "queue"). Same scope choice as
// Solace: no "reuse existing listener" path, since that hasn't been confirmed
// to actually exist in this branch's live UI for any MULTIPLE_SELECT_LISTENER
// trigger yet (MQTT's static model promised one that wasn't there live).
export default function createTests() {
    test.describe.serial('Solace JMS Integration Tests', {
    }, async () => {
        initTest();
        const listenerName = 'orderSolaceJmsListener';
        const queueName = 'order-events';

        test('Create Solace JMS Integration', async ({ }, testInfo) => {
            const testAttempt = testInfo.retry + 1;
            console.log('Creating a new service in test attempt: ', testAttempt);

            const artifactWebView = await createArtifactAndGetWebview('Solace (JMS) Integration', 'trigger-solace.jms');
            ...
            await form.fill({
                values: {
                    url: { type: 'cmEditor', value: '"smf://localhost:55555"', additionalProps: { switchMode: 'expression-mode' } },
            ...
            await projectExplorer.findItem([DEFAULT_PROJECT_NAME, `Solace (JMS) Event Integration`], 30000);
        ...
        test('Delete Solace JMS Integration', async ({ }, testInfo) => {
            const testAttempt = testInfo.retry + 1;
            console.log('Deleting Solace JMS integration in test attempt: ', testAttempt);

            await getWebview(BI_INTEGRATOR_LABEL, page);
            await deleteArtifactFromTree([DEFAULT_PROJECT_NAME, `Solace (JMS) Event Integration`]);
        });
    });
}
